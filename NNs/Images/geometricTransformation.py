...
def run(dataset_name, dataset, setting):
    print(dataset_name + " Geometric Transformation")
    ...
    if dataset_name == 'MNIST':
        return training_accuracies, testing_accuracies, difference_in_accuracies
    ...
    for train_indices, val_indices in kf.split(data):
        ...
        network = Net(setting.in_channels, setting.out_channels, setting.kernel_size, setting.pool_size, setting.hidden_layer_sizes, setting.output_size, setting.stride, dropout = setting.dropout_layer)
        if torch.cuda.is_available():
        ...
        for epoch in range(setting.number_of_epochs):  # Adjust as needed
            # Forward pass
    ...
    return create_numeric_run_object("Geometric Transformation", seed, start_time, end_time, setting,
