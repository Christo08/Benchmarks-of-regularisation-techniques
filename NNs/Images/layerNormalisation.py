...
def run(dataset_name, dataset, setting):
    print(dataset_name+" Layer Normalisation")
    ...
    for train_indices, val_indices in kf.split(data):
        ...
        network = Net(setting.in_channels, setting.out_channels, setting.kernel_size, setting.pool_size, setting.hidden_layer_sizes, setting.output_size, setting.stride, layer_norm=True)
        if torch.cuda.is_available():
        ...
        for epoch in range(setting.number_of_epochs):
            ...
            if epoch % setting.log_interval == 0:
                print("Fold %s, Epoch %s, Training loss %s, Testing loss %s, Training accuracy %s, Testing accuracy %s" % (fold, (epoch+1), training_loss.item(), testing_loss.item(),training_accuracy,testing_accuracy))
            if epoch == setting.number_of_epochs-1:
                print("Fold %s, Epoch %s, Training loss %s, Testing loss %s, Training accuracy %s, Testing accuracy %s" % (fold, (epoch+1), training_loss.item(), testing_loss.item(),training_accuracy,testing_accuracy))
        fold +=1
    ...
    return create_numeric_run_object("Layer Normaliser", seed, start_time, end_time, setting,
