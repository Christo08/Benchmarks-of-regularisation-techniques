...
def run(dataset_name, dataset, setting):
    print(dataset_name+" weight normalisation")
    ...
    for train_indices, val_indices in kf.split(data):
        ...
        network = Net(setting.in_channels, setting.out_channels, setting.kernel_size, setting.pool_size, setting.hidden_layer_sizes, setting.output_size, setting.stride, weight_norm=True)
        if torch.cuda.is_available():
    ...
    return create_numeric_run_object("Weight Normalisation", seed, start_time, end_time, setting,
