...
def run(dataset_name, settings, training_set, validation_set):
    print(dataset_name + " pruning run")
    ...
    monitor = Monitor("Pruning", dataset_name, seed, loss_function, settings.log_interval, x_validation, y_validation)
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(features_tensor)):
        ...
        for epoch in range(settings.number_of_epochs):
            ...
            if epoch % settings.prune_epoch_interval == 0 and epoch != 0:
