...
def run(dataset_name, dataset, setting):
    print(dataset_name + " Batch Normalisation")
    ...
    for train_indices, val_indices in kf.split(data):
        ...
        x_testing, y_testing = data[val_indices], labels[val_indices]
        ...
        network = Net(setting.in_channels, setting.out_channels, setting.kernel_size, setting.pool_size,
                      setting.hidden_layer_sizes, setting.output_size, setting.stride, batch_norm=True)
        if torch.cuda.is_available():
        ...
        for epoch in range(setting.number_of_epochs):
            ...
            if epoch % setting.log_interval == 0:
                print(
                    ...
                    fold, (epoch + 1), training_loss.item(), testing_loss.item(), training_accuracy, testing_accuracy))
        fold += 1
    ...
    return create_numeric_run_object("Batch Normaliser", seed, start_time, end_time, setting,
                                     training_accuracies, testing_accuracies, validation_accuracies,
                                     difference_in_accuracies)
