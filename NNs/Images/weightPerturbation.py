...
def run(dataset_name, dataset, setting):
    print(dataset_name+" weight perturbation")
    ...
    for train_indices, val_indices in kf.split(data):
        ...
        for epoch in range(setting.number_of_epochs):
            ...
            if epoch % setting.weight_perturbation_epoch_interval() == 0 and epoch !=0:
                network.perturb_weights(perturbation_factor=setting.weight_perturbation_amount)
    ...
    return create_numeric_run_object("Weight Perturbation", seed, start_time, end_time, setting,
